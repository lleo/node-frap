...
var RFrameStream = exports.RFrameStream = function RFrameStream(frap, framelen) {
  if (RFrameStream.VERBOSE) log("RFrameStream: framelen=%d;", framelen)
  Stream.call(this)
  ...
  function onPart(buf, pos) {
    if (RFrameStream.VERBOSE) log("RFrameStream: onPart: buf.length=%d; pos=%d;", buf.length, pos)
...
function RFrameStream__destroy() {
  ...
  if (this.paused) {
    //console.error("Warning: RFrameStream.prototype.destroy called while paused; resuming it.")
    //console.error("         This probaly means you piped this RFrameStream to a non-Frap Stream.")
    this.resume()
  }
