...
var RFrameStream = exports.RFrameStream = function RFrameStream(frap, framelen) {
  ...
  function onPart(buf, off) {
    ...
    if (npos > self.framelen) {
      //throw new Error(format("framelen exceeded: framelen=%d; npos=%d;",
      //                       self.framelen, npos))
      self.emit('error', format("framelen exceeded: framelen=%d; npos=%d;",
                                self.framelen, npos))
      return
    }
    ...
    if (self.paused) {
      this.paused_queue.push([buf, off])
    }
    else {
      self.emit_data(buf, off)
    }
  }
  function onError(e) {
...
RFrameStream.prototype.emit_data = function(buf, off) {
  this.data_cnt += 1
  this.pos += buf.length
  this.emit('data', buf, off)
...
function RFrameStream__pause(encoding) {
  ...
  if (this.paused) log("RFrameStream__pause: already paused")
...
function RFrameStream__resume(encoding) {
  ...
  if (!this.paused) log("RFrameStream__resume: not paused ?!?")

  this.paused = false

  var qlen = this.paused_queue.length
  var self = this
  for (var i=0; i< qlen; i++) {
    var part = this.paused_queue.shift() //oh for the want of de-structuring :)
      , buf = part[0]
      , pos = part[1]

    this.emit_data(buf, pos)

    //this.emit 'data' may call paused again
...
WFrameStream.prototype.finished = function (ondrain) {
  ...
  //this.emit('finished', this.pause_cnt
  //                    , this.resume_cnt
  //                    , this.write_cnt
  //                    , this.wrote_cnt
  //                    , this.drain_cnt)
