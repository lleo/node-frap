...
function WFrameStream__write(buf, enc) {
  ...
  if (this.sofar === this.framelen) {
    ...
    var self = this
    if (flushed) {
      //wstream.close() emits a 'close' event
      // in frap.send() wstream 'close' events cause a frap to emit 'drain'
      // frap 'drain' events can cause a frap.send()
      // this would all be synchronouse causing stack frame to explode
      // wrapping wstream.close in a nextTick make this process asynchronous
