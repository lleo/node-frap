...
var Frap = function Frap(sk, noframes) {
  ...
  this.paused = false
  //this.eventq = [] //Pending Input Events
  ...
  //this.sendq = [] //Pending Writes
  this.sendq = new Dequeue() //Pending Writes
  ...
  switch (event) {
    ...
    case 'frame':
      ...
      if (this.listeners('data').length > 0) {
        var fbuf
        //concat can be very expensive; only do it if someone is listening
...
Frap.prototype.destroy = function Frap__destroy() {
  ...
  if (this.sendq) this.sendq.empty()
  //delete this.sk
...
Frap.prototype.destroySoon = function Frap__destroySoon() {
  ...
  var self = this
  if (this.sendq.length > 0 || this.wstream) {
...
Frap.prototype.pipe = function Frap__pipe(dst) {
  ...
  function pipeOnHeader(rstream, framelen){
    ...
    rstream.once('close', function(){
      ...
      wstream.destroySoon() //only emits 'close' when it is drained
    })
...
//Frap.prototype.sendFrame = sendFrameWstreamSendq
...
function sendFrameWstreamSendq() {
  var self = this
    , bufs_ = Array.prototype.slice.call(arguments)

  if (bufs_.length === 1 && Array.isArray(bufs_[0])) {
    bufs_ = bufs_[0]
  }

  this.sendq.push(bufs_)

  if (this.wstream) { //currently writing
    if (Frap.VERBOSE) log("Frap__sendFrame: wstream exists;")
    return false
  }
  ...
  function sendFrames() {
    var sent=true
    while (sent && self.sendq.length > 0) {
      var bufs = self.sendq.shift()

      var framelen = 0
      for (var j=0; j<bufs.length; j++) {
        assert(Buffer.isBuffer(bufs[j]), "every element of bufs must be a Buffer")
        framelen += bufs[j].length
      }

      var i = 0
        , wstream = self.createWriteStream(framelen)

      function writeFrameBufs() {
        var sent_=true //either first time or on 'drain' event
        while (sent_ && i<bufs.length) {
          sent_ = wstream.write(bufs[i])
          i += 1
        }
        ...
        if (!sent_) {
          ...
          wstream.once('drain', function(){
            ...
            if (i < bufs.length) writeFrameBufs()
            else wstream.destroy()
          })
        }
        else { // i === bufs.length
          wstream.destroy() //emits 'close' which frees self.wstream
        }

        return sent_
      } //writeFrameBufs

      sent = writeFrameBufs() //true means it was sent immediately

      if (!sent) {
        wstream.once('close', sendFrames)
      }
    } //while send && sendq

    if (has_drained && self.sendq.length === 0) {
      self.emit('drain')
    }
  ...
  function onHeader(rstream, framelen) {
    ...
    function _onData(buf, off) {
      ...
      if (off+buf.length === framelen) {
        //possible optimization: emit directly; don't pass to submit/dispatch
        self.emit('frame', bufs, framelen)
        //self._submit('frame', bufs, framelen)

        //possible optimization: only concat if someone listens to 'data'
