...
Frap.prototype.sendFrame = function Frap__sendFrame() {
  ...
  var cb_
  if (bufs_.length > 1 && typeof bufs_[bufs_.length-1] === 'function') {
    cb_ = bufs_.pop()
  }
  ...
  this.pending_frames.push([bufs_, cb_])
  ...
  function sendFrames() {
    var sent=true, drained=false
    while (sent && self.pending_frames.length > 0) {
      var frm = self.pending_frames.shift()
        , bufs = frm[0]
        , cb   = frm[1]
    ...
    } //while send && pending_frames

    if (drained && self.pending_frames.length === 0) {
      self.emit('drain')
    }
  } //sendFrames
