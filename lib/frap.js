...
Frap.prototype.sendFrame = function Frap__sendFrame() {
  ...
  function onFrameSent() {
    //log("onFrameSent: pending=%d", self.pending.length)
    assert(self.sending, "sending flag not set")
    var sent = true, n=0
    while (sent && self.pending.length > 0) {
      sent = self._sendFrame(self.pending.shift())
      n += 1
    }
    if (!sent) { //pending remaining
      log("onFrameSent: pending remaining=%d", self.pending.length)
      self.once('_frameSent', onFrameSent)
      return
    }
    //log("onFrameSent: emitting 'drain'")
    self.sending = false
    self.emit('drain')
  }

  var sent = this._sendFrame(bufs)
  if (!sent) {
    this.sending = true
...
function Frap__createWriteStream(framelen) {
  ...
  this._wstream.once('finished', function() {
    //log("createWriteStream: this._wstream.once 'finished'")
