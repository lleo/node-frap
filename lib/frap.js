...
Frap.prototype.readFrame = function Frap__readFrame(buf, off) {
  ...
  if (datalen === framelen) { //whole frame came in first read
    log(format("readFrame: datalen === framelen; %d === %d", datalen, framelen))
...
Frap.prototype.readData = function Frap__readData(buf) {
  ...
  var start, end
  if (buf.length === rem) {
    log(format("readData: buf.length === rem; %d === %d", buf.length, rem))
    ...
    //THIS CODE PATH IS NOT TOUCHED BY MY TESTS; ie it propably doesn't work
    log(format("readData: buf.length > rem; %d > %d", buf.length, rem))
    ...
    this.readFrame(buf, buf.length-rem)
  }
  else if (buf.length < rem) {
    log(format("readData: buf.length < rem; %d < %d", buf.length, rem))
