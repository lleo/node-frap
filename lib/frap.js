...
exports.RFrameStream = RFrameStream
exports.WFrameStream = WFrameStream
...
Frap.prototype.send = function Frap__send() {
  ...
  var wrote=true, i=0, sofar=0, wstream = this.createWriteStream(tot, cb)
  ;(function do_write() {
    if (Frap.VERBOSE)
...
function onData(buf) {
  if (!this._cur_recv) {
    ...
    , hdrbuf: undefined     //is partial header buffer
...
Frap.prototype.readHeader = function Frap__readHeader(buf) {
  var nbuf = new Buffer(this._cur_recv.hdrbuf.length + buf.length)

  this._cur_recv.hdrbuf.copy(nbuf) //1, 2, or 3 bytes copied
  buf.copy(nbuf, this._cur_recv.hdrbuf.length)

  this._cur_recv.hdrbuf = undefined
...
Frap.prototype.readFrame = function Frap__readFrame(buf) {
  ...
  if (datalen === framelen) { //whole frame came in first read
    ...
    //this._cur_recv.header   = "frame"
    //this._cur_recv.hdrbuf   = undefined
    this._cur_recv.framelen = undefined
    //this._cur_recv.sofar    = 0
    this._cur_recv.nreads   = 0

    //delete this._cur_recv
    //onData(buf.slice(4+framelen))
    ...
    //this._cur_recv.hdrbuf   = undefined
    //this._cur_recv.framelen = framelen
