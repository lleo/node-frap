...
var SimpleFrap = function SimpleFrap(src) {
  ...
  this.sending = false
  this.sendq = new Dequeue()

  var self = this

  function onSrcData(buf) {
    if (SimpleFrap.VERBOSE) log("Frap: onSrcData: state=%s; buf.length=%d;", self.ps.state, buf.length)
    self._parse(buf)
  }

  function onSrcError(err) {
    if (Frap.VERBOSE) log("Frap: onSrcError:", err)
    self.emit('error', err)
    self.destroy()
  }
  ...
  Object.keys(this.src_listeners).forEach(function(k){
    self.src.on(k, self.src_listeners[k])
  })
...
SimpleFrap.prototype.sendFrame =
SimpleFrap.prototype.send = function(buf) {
  var self = this
  ...
  function _send(b) {
    ...
    hdrbuf.writeUInt32BE(b.length, 0)
    self.src.write(hdrbuf)
    var sent = self.src.write(b)
    return sent
  }
  ...
  function sendFrames() {
    ...
    while (sent && self.sendq.length > 0) {
      self.sending = true
      var buf = self.sendq.shift()
      sent = _send(buf)
      if (!sent) {
        draining = true
        self.src.once('drain', function() {
          //log("sendFrames: drained; #sendq=%d;", self.sendq.length)
          has_drained = true
          self.sending = false
      ...
      else self.sending = false
    } //while
    if (self.sendq.length === 0 && has_drained) {
      has_drained = false
      self.emit('drain')
    }
    return sent
